...
def calc_prob(player, p, n, k):
    ...
    print (' The probability of player' , player, 'making' ,k, 'shots in an average game is %1.1f%% ' %prob)
    print (' \n')
...
k = 9
prob_A = calc_prob('A', p_A, n_A, k)
prob_B = calc_prob('B', p_B, n_B, k)
```
...
def mean_sd(player, n, p):
    ...
    print ('Player', player, 'will make an average of', np.round(mu),'+/- ',np.round(sigma), 'shots per game')
    print ('\n')
...
print (np.round(mean), np.round(sd**0.5))
